...
在任意普通应用的输入框按 **Win+H** 或 **Ctrl+Win+H**，白绿小窗弹出并开始录音。再按其中任一组合或中间麦克风按钮暂停，暂停后小窗保留，点击麦克风可继续；停顿时会逐句输入。拖动顶部可移动小窗，底部可选择和刷新麦克风，DJI 是默认选择。

**×** 立即停录并收起小窗，后台仍待命；**Esc** 取消尚未输入的部分，已输入文字保留。只有托盘“退出”或脚本 Stop 才退出程序并释放模型，文字不会自动按回车发送。
